...
#
# Hard Coded constants for DECam, a set of list/arrays and dictionaries
# are hard-coded here
#
#
# CCDSECTIONS:
...
# | awk ' \
# {printf \"%s %-30s %2d %12.2f %12.2f %14.8f %14.8f\n\",$1,$2,$3,$4,$5,$6,$7}'
#
...
def drawDECamCCDs_Polygon(
        x0, y0, trim=True, rotate=True, label=False, **kwargs):
    """
    Draws DECam CCDs shapes using matplotlib Polygons function
    on the current plot
    """
...
def drawDECamCorners_Sky(ra, dec, pixscale=0.27,
                         label=False, units='deg', **kwargs):
    """
...
def drawDECamCCDs_Sky(ra, dec, trim=True, pixscale=0.27,
                      label=False, **kwargs):
    """
    ...
    for k, v in SECTIONS.items():
        ...
        if label:  # Probably will never use
            ...
            ax.text(0.5 * (r2 + r1), 0.5 * (d2 + d1), "CCD%s" %
                    k, ha='center', va='center')
    return
...
def getDECamCCDs(header, plot=False, trim=True, **kwargs):
    ...
    for k, v in SECTIONS.items():
        ...
        if plot:
            ...
            r_plot, d_plot = DECam_wcs.image2sky(
                DECam_x, DECam_y)  # Into RA,DEC
...
def PEllipse(xxx_todo_changeme, xxx_todo_changeme1,
             resolution=100, angle=0.0, **kwargs):
...
def PCircle(xxx_todo_changeme4, radius, resolution=100, **kwargs):
    """ Draw a circle as a Polygon too """
    (xo, yo) = xxx_todo_changeme4
    pi = math.pi
#    cos = math.cos
#    sin = math.sin
    t = 2 * pi / resolution * numpy.arange(resolution)
...
def run_tests():
    ...
    for r in ra:
        for d in dec:
            drawDECamCCDs_Sky(r, d, trim=True, pixscale=0.27,
                              label=False, color='k', ls='-')
    plt.xlim(r1, r2)
    plt.ylim(d1, d2)
    plt.xlabel('R.A.')
    plt.ylabel('Dec.')
